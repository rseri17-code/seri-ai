...
- `8860517 Refresh Claude handoff after profile graph indexing`
...
- Search retrieval covers 68 canonical queries.
- Ask deterministic fixtures cover 116 passing cases.
...
- Ask scorecard evidence now reflects 116 passing fixtures, the versioned persona contract, intent-aware follow-up questions, and review-quorum routing, and `validate:ask-quality` guards the fixture count against drifting from the eval report.
- Product Excellence report current scorecard now reflects 116 Ask fixtures, persona/follow-up contracts, review-quorum routing, current accessibility coverage, latest performance numbers, and integrated portrait provenance. `validate:deployment` and `validate:handoff` now guard the report and handoff against stale evidence claims.
