...
const referenceSources = [
  ...
  },
  {
    ...
    description: "Public thesis: for services, context is the system map and deployments; for batch, context is the execution graph — which jobs depend on which. No employer data.",
    content:
      "Batch Intelligence is the public thesis that for services, context is the system map and deployments, while for batch, context is the execution graph: which jobs depend on which. Batch context layer. Batch execution graph. A failed job is only the visible endpoint. Dependency-aware context connects what ran before it, what consumes its output, and what evidence is still missing. The inspectable six-step chain runs from failed workload through predecessor, affected successors, blast radius, evidence packet, and human-reviewed recovery. GROUNDED RCA is allowed only when the checked-in evidence covers the trigger, dependency path, affected successors, and review gate. EXPLICIT UNKNOWN is required when any causal link is unsupported. The published Owl paper grounds the dependency model only. OI-ROOM-001 is a synthetic deterministic fixture, not a reproduction of an employer system. Canonical deep dive: /framework#batch-intelligence.",
