...
function patternAsset(pattern: Pattern): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: pattern.related.filter((item) => item.startsWith("/investigation-room") || item.startsWith("/evals") || item.startsWith("/library")),
...
function productAsset(product: (typeof products)[number]): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    patterns: patterns.filter((pattern) => contentMatches(content, [pattern.title, ...pattern.tags])).map((pattern) => `/patterns/${pattern.slug}`),
    artifacts: ["/investigation-room", "/evals", "/library"],
    products: [],
    askQuestions: [],
    content,
...
function principleAsset(principle: Principle): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: principle.related.filter((item) => item.startsWith("/investigation-room") || item.startsWith("/evals") || item.startsWith("/library")),
    products: [],
    askQuestions: [],
    content,
