...
import { asSentence, formatNaturalList, getPublishedWikiNotes, proseList, type WikiNote } from "./content";
...
function articleAsset(article: Article): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: contentMatches(content, ["incident", "room", "replay", "eval"]) ? ["/investigation-room", "/ask"] : [],
...
function patternAsset(pattern: Pattern): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: pattern.related.filter((item) => item.startsWith("/investigation-room") || item.startsWith("/ask") || item.startsWith("/library")),
...
function productAsset(product: (typeof products)[number]): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    patterns: patterns.filter((pattern) => contentMatches(content, [pattern.title, ...pattern.tags])).map((pattern) => `/patterns/${pattern.slug}`),
    artifacts: ["/investigation-room", "/ask", "/library"],
    products: [],
    askQuestions: [],
    content,
...
function principleAsset(principle: Principle): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: principle.related.filter((item) => item.startsWith("/investigation-room") || item.startsWith("/ask") || item.startsWith("/library")),
    products: [],
    askQuestions: [],
    content,
...
function wikiAsset(note: WikiNote): PublishingAsset {
  ...
  const asset: PublishingAsset = {
    ...
    artifacts: contentMatches(content, ["operations room", "replay", "eval"]) ? ["/investigation-room", "/ask"] : [],
...
export function buildRssFeed(siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://seri-ai.vercel.app") {
