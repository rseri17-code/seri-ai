...
const ContactSchema = z.object({
  ...
  artifactsInspected: z.string().max(1000).optional(),
  reviewDisposition: z.string().max(80).optional(),
  ...
  firstImpressionVerdict: z.string().max(120).optional(),
  personWorkFit: z.string().max(120).optional(),
  thesisFit: z.string().max(120).optional(),
  proofRouteFit: z.string().max(120).optional(),
  artifactRecall: z.string().max(120).optional(),
  demoSignal: z.string().max(120).optional(),
  ...
  if (supabase && runtime.contactPersistenceConfigured) {
      ...
      parsed.data.kind === "beta-feedback"
        ...
        : parsed.data.kind === "practitioner-review"
          ? [
              ...
              `Artifacts inspected: ${parsed.data.artifactsInspected ?? "unspecified"}`,
              `Review disposition: ${parsed.data.reviewDisposition ?? "unspecified"}`,
              ...
              `First-impression verdict: ${parsed.data.firstImpressionVerdict ?? "unspecified"}`,
              `Person-work fit: ${parsed.data.personWorkFit ?? "unspecified"}`,
              `Thesis fit: ${parsed.data.thesisFit ?? "unspecified"}`,
              `Proof route fit: ${parsed.data.proofRouteFit ?? "unspecified"}`,
              `Artifact recall: ${parsed.data.artifactRecall ?? "unspecified"}`,
              `Demo signal: ${parsed.data.demoSignal ?? "unspecified"}`,
    ...
    const row = {
      ...
      metadata: {
        ...
        artifactsInspected: parsed.data.artifactsInspected,
        reviewDisposition: parsed.data.reviewDisposition,
        ...
        firstImpressionVerdict: parsed.data.firstImpressionVerdict,
        personWorkFit: parsed.data.personWorkFit,
        thesisFit: parsed.data.thesisFit,
        proofRouteFit: parsed.data.proofRouteFit,
        artifactRecall: parsed.data.artifactRecall,
        demoSignal: parsed.data.demoSignal,
