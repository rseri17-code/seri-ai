...
| Ask quality | Reviewer-labeled Ask sessions across profile, doctrine, routing, unknowns, and boundaries. | Deterministic fixtures, refusal behavior, and the Ask live review packet pass validation. | Run `/publication-pack/ask-ravi-live-review-packet.md` to compare grounded answer paths using safe metadata only. | Ask cannot cite sources or separate fact from inference. |
