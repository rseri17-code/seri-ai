...
| Identity asset | Approved portrait or identity asset connected to the person-work-evidence path. | Public routes link Ravikanth to the work, a durable non-photographic public identity mark exists, and `/identity/portrait-intake.md` defines the approved portrait intake contract. | Add an approved portrait photo through the intake contract when a durable source image is available, then run external first-impression review. | The site feels anonymous or uses an unapproved image. |
