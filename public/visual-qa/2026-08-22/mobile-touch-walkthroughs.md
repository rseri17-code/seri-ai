...
| `/framework` | Inspect the ten-layer model, the argument for the category, and the market signals paired with their falsification questions. | Responsive route structure, evidence posture, and retrieval routing are validated. | **STALE:** this row described `/radar`, retired on 2026-08-29 with its content folded into `/framework`. The folded sections have not been re-walked on a device; that recapture is outstanding. |
