...
| Ask Ravi usefulness and safety | 108 deterministic fixtures pass; fallback, refusal, routing, citation, AI-disclosure, safe operational metadata, versioned persona contract, intent-aware follow-up questions, review-quorum routing, and observability contracts are validated. | Strong deterministic beta, needs live reviewer-labeled answer quality and production retrieval evidence. |
...
| Performance | `npm run build` passed performance budgets; latest production build reports home first-load JS at 107 kB and Operations Room first-load JS at 227 kB. | Acceptable, keep watching Operations Room payload and real-device interaction latency. |
