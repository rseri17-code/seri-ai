...
| UX and visual quality | Homepage first impression, Operations Room graph, evidence taxonomy, Ask Ravi answer packet, reviewer paths, and viewport contract validation for five critical routes are coherent. | Source-level viewport contracts do not replace rendered screenshot review. Some dense pages still need premium interaction polish. | Use visual audits of `/`, `/investigation-room`, `/ask`, `/work`, and `/wiki/operational-intelligence-canonical-doctrine` at mobile/tablet/desktop. |
