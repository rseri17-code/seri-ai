...
| UX and visual quality | Homepage first impression, Operations Room graph, evidence taxonomy, Ask Ravi answer packet, and reviewer paths are coherent. | Some dense pages still read like reference material before they feel like a premium interactive system. | Use visual audits of `/`, `/investigation-room`, `/ask`, `/work`, and `/wiki/operational-intelligence-canonical-doctrine` at mobile/tablet/desktop. |
...
| Mobile experience | Responsive layout contracts and accessibility validation exist; homepage and Operations Room use mobile-safe grids. | Needs real visual inspection at 390px and tablet width for dense interactive flows. | Capture screenshots and fix text density, tap targets, and graph framing issues from the live rendered site. |
...
3. **Doctrine evidence depth**: add citations and field-note evidence that distinguish established foundations from Ravikanth's synthesis.
4. **Publishing migration**: move high-change essays, patterns, and notes out of large TypeScript arrays into scalable content files.
5. **Production observability proof**: document privacy-safe SLO evidence after Vercel beta traffic.
6. **Security review**: run dependency audit, verify env scoping, and document secret rotation.
7. **Mobile manual audit**: capture real viewport evidence for homepage, Ask Ravi, Operations Room, Work, and Doctrine.
8. **External proof loop**: collect practitioner review through the existing contact/review mechanism and convert useful feedback into evidence-pack updates.
